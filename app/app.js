 ...
 var MAP_CENTER = [31.765, 35.17]; //TODO: replace with a specific for the city
 var DEFAULT_ZOOM = 13; //TODO: move to envs
 ...
 var addAttachmentsTooltips = function() {
     $("[rel='tooltip']").tooltip({
         'placement': 'bottom'
     });
 };

 var addItemHover = function() {
     $(".item").hover(
         function() {
             $(this).css("background", "#fff");
             $(this).next(".details").css("background", "#fff");
         }, //#f7f7f9
         function() {
             $(this).css("background", "");
             $(this).next(".details").css("background", "");
         }
     );
 };

 var addDetailsHover = function() {
     $(".details").hover(
         function() {
             $(this).css("background", "#fff");
             $(this).prev(".item").css("background", "#fff");
         },
         function() {
             $(this).css("background", "");
             $(this).prev(".item").css("background", "");
         }
     );
 };
 ...
 function find_gush(gush_id) {
     g = gushim.features.filter(
         function(f) {
             return (f.properties.Name == gush_id);
         }
     );
     return g[0];
 }
 var _addressErrorHandler = function(err) {
     $('#addr-error-p').html(err);
         ...
         function(r) {
             ...
             if (r['status'] == 'OK' && r['results'].length > 0) {
                 ...
                 if (r['results'][0]['types'].length == 2 &&
                     ...
                     _addressErrorHandler('כתובת שגויה או שלא נמצאו נתונים');
                 } else {
                     ...
                     if (gid && gid.length > 0) {
                         ...
                         _addressErrorHandler('לא נמצא גוש התואם לכתובת'); // TODO: when enabling multiple cities change the message to point users to try a differenct city
                     }
                 }
             } else if (r['status'] == 'ZERO_RESULTS') {
                 _addressErrorHandler('כתובת שגויה או שלא נמצאו נתונים');
             } else {
                 _addressErrorHandler('חלה שגיאה בחיפוש הכתובת, אנא נסו שנית מאוחר יותר');
             }
         }
     )
         .fail(
 ...
 $(document).ready(function() {
     ...
     Path.map("#/gush/:gush_id").to(
         function() {
             $("#docModal").modal('hide'); //TODO: no docModal in index html currently - did you mean doc-modal?
